...
NAMING_CONVENTIONS = {
    'snake_case': 'Snake Case (user_name)',
    'kebab_case': 'Kebab Case (user-name)', 
    'camelCase': 'Camel Case (userName)',
    'PascalCase': 'Pascal Case (UserName)',
    'UPPER_CASE': 'Upper Case (USER_NAME)',
    'lower_case': 'Lower Case (user name → user_name)',
...
def convert_naming_convention(name: str, convention: str) -> str:
    ...
    Returns:
        Converted name
    """
    if not name or convention == 'original':
        return name
    
    # First, split the name into words (handle various separators)
    import re
    ...
    if convention == 'snake_case':
        ...
        return '_'.join(words)  # Same as snake_case but explicit
    else:
        return name

def format_comma_separated_list(items: List[str], comma_format: str, indent: str = "  ") -> str:
    """
    Format a comma-separated list with the specified comma format.
    
    Parameters:
        items: List of items to format
        comma_format: 'trailing' or 'leading'
    ...
    if comma_format == 'leading':
        # Leading commas: all items should align at the same column position
        # First item gets extra spaces to align with comma + space of subsequent items
        formatted = []
        for i, item in enumerate(items):
            stripped_item = item.lstrip()
            if i == 0:
                # First item: add extra spaces to align with subsequent comma + space
...
def sanitize_identifier(name: str, sanitize_method: str = "underscore", naming_convention: str = "original") -> str:
    """
    Sanitize SQL identifiers (table names, column names) to handle spaces and special characters,
    and apply naming convention conversion.
    ...
    Parameters:
        ...
        naming_convention: Naming convention to apply after sanitization
    
    Returns:
        Sanitized and formatted identifier
    """
    if not name:
        return name
    ...
    # Apply naming convention first (this handles word separation)
    converted_name = convert_naming_convention(name, naming_convention)
    
    if sanitize_method == "underscore":
        # For kebab-case, we need special handling since Databricks doesn't support hyphens
        if naming_convention == "kebab_case":
            # Convert kebab-case to snake_case for Databricks compatibility
            converted_name = converted_name.replace('-', '_')
        
        # Replace remaining special characters with underscore (keep alphanumeric and underscore/hyphen)
        # This is especially important for catalog and schema names in Databricks
        converted_name = re.sub(r'[^a-zA-Z0-9_]', '_', converted_name)
        
        # Remove multiple consecutive underscores
        converted_name = re.sub(r'_{2,}', '_', converted_name)
        
        # Remove leading/trailing underscores
        converted_name = converted_name.strip('_')
        
        return converted_name
    else:  # backtick
        return f"`{converted_name}`"
...
def format_column_definition(col_name: str, data_type: str, not_null: bool = False, 
                           ...
                           max_name_width: int = 20, max_type_width: int = 15,
                           indent: str = "  ") -> str:
    """
    ...
    Parameters:
        ...
        indent: Indentation string for the column definition
    ...
    column_def = f"{indent}{padded_name} {padded_type}"
...
def generate_ddl(model_data: Dict[str, Any], include_options: Dict[str, bool]) -> str:
    ...
    naming_convention = include_options.get('naming_convention', 'original')
    comma_format = include_options.get('comma_format', 'trailing')
...
-- For more information, see: https://docs.databricks.com/en/tables/constraints.html
    ...
    for entity in entities:
        ...
        for attr in attributes:
            ...
            sanitized_attr_name = sanitize_identifier(attr_name, sanitize_method, naming_convention)
    ...
    for entity in entities:
        ...
        sanitized_entity_name = sanitize_identifier(entity_name, sanitize_method, naming_convention)
        
        # Build fully qualified table name
        full_table_name = sanitized_entity_name
        if schema_name:
            full_table_name = f"{sanitize_identifier(schema_name, sanitize_method, naming_convention)}.{full_table_name}"
        if catalog_name:
            full_table_name = f"{sanitize_identifier(catalog_name, sanitize_method, naming_convention)}.{full_table_name}"
        ...
        # Add table columns using comma formatting
        table_stmt += format_comma_separated_list(columns, comma_format)
        ...
        if include_options.get('include_pk', True) and primary_keys:
            ...
            constraint_def = f"  CONSTRAINT {pk_name} PRIMARY KEY ({pk_cols})"
    ...
    if include_options.get('include_foreign_keys', True):
        ...
        for entity in entities:
            ...
            for attr in entity.get('attributes', []):
                if attr.get('metadata', {}).get('PK', False):
                    sanitized_attr_name = sanitize_identifier(attr.get('name', ''), sanitize_method, naming_convention)
        ...
        for relationship in relationships:
            ...
            sanitized_source_name = sanitize_identifier(source_name, sanitize_method, naming_convention)
            sanitized_target_name = sanitize_identifier(target_name, sanitize_method, naming_convention)
            sanitized_source_attrs = [sanitize_identifier(attr, sanitize_method, naming_convention) for attr in source_attrs]
            sanitized_target_attrs = [sanitize_identifier(attr, sanitize_method, naming_convention) for attr in target_attrs]
            ...
            if schema_name:
                schema_sanitized = sanitize_identifier(schema_name, sanitize_method, naming_convention)
                full_source_name = f"{schema_sanitized}.{full_source_name}"
                full_target_name = f"{schema_sanitized}.{full_target_name}"
            
            if catalog_name:
                catalog_sanitized = sanitize_identifier(catalog_name, sanitize_method, naming_convention)
    ...
    # Include foreign key constraints as comments if option is enabled
        ...
        for relationship in relationships:
            ...
            if not (source_name and target_name and source_attrs and target_attrs):
                ...
                sanitized_source_name = sanitize_identifier(source_name, sanitize_method, naming_convention)
                sanitized_target_name = sanitize_identifier(target_name, sanitize_method, naming_convention)
                sanitized_source_attr = sanitize_identifier(source_attrs[0], sanitize_method, naming_convention) if source_attrs else ""
                sanitized_target_attr = sanitize_identifier(target_attrs[0], sanitize_method, naming_convention) if target_attrs else ""
...
def main():
    ...
    with col1:
        ...
        selected_naming_convention = st.selectbox(
            "Choose naming convention for tables and columns:",
            options=list(NAMING_CONVENTIONS.keys()),
            index=list(NAMING_CONVENTIONS.keys()).index('original'),  # Default to Keep Original
            format_func=lambda x: NAMING_CONVENTIONS[x],
            help="Convert field names to the selected naming convention. 'Keep Original' preserves names as-is."
        )
        
        options["naming_convention"] = selected_naming_convention
        
        # Show example conversion
        if selected_naming_convention != 'original':
            example_names = ["Customer ID", "orderDate", "ProductName", "user_profile"]
            converted_examples = [convert_naming_convention(name, selected_naming_convention) for name in example_names]
            example_text = " | ".join([f"`{orig}` → `{conv}`" for orig, conv in zip(example_names, converted_examples)])
            st.info(f"**Example conversions:** {example_text}")
        ...
        # Special handling for kebab-case
        if selected_naming_convention == 'kebab_case':
            st.warning("⚠️ **Kebab-case note:** Hyphens will be converted to underscores for Databricks compatibility.")
        
        # Comma Formatting
        st.subheader("🔗 Comma Formatting")
        selected_comma_format = st.selectbox(
            "Choose comma format for column lists:",
            options=list(COMMA_FORMATS.keys()),
            index=0,  # Default to trailing
            format_func=lambda x: COMMA_FORMATS[x],
            help="Choose whether commas appear at the end of lines (trailing) or beginning of lines (leading)."
        )
        
        options["comma_format"] = selected_comma_format
        
        # Show example format
        example_columns = ["customer_id BIGINT NOT NULL", "customer_name STRING", "created_date TIMESTAMP"]
